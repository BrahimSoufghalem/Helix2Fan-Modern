...
def read_projections(folder, indices):
    ...
    # Get the relevant file names, sorted correctly by DICOM InstanceNumber
    raw_files = [f for f in os.listdir(folder) if f.endswith(".dcm")]
    print("⏳ Sorting DICOM projections chronologically via InstanceNumber...")
    file_info = []
    for f in raw_files:
        ds_hdr = pydicom.dcmread(os.path.join(folder, f), stop_before_pixels=True)
        file_info.append((f, ds_hdr.InstanceNumber))
...
def read_dicom(parser):
    ...
    dv_rebinned = np.sign(z_positions[-1] - z_positions[0]) * abs(dv_rebinned)
    if dv_rebinned == 0: dv_rebinned = 1
    nz_rebinned = abs(int((z_positions[-1] - z_positions[0]) / dv_rebinned))
    hu_factor = float(data_headers[0][0x70411001].value)
    rotview = abs(int(len(angles) / ((angles[-1] - angles[0]) / (2 * np.pi))))
    pitch = abs(pitch)
